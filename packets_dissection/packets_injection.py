...
for pkt in packets:
    ...
    print()
    modified_pkt = bytes(ether_header)
    ...
    if ether_header.type_str == 'IPv4':
        ...
        print("ARP op code: ", arp_header.op)
        # Change op data field
        print()
        print("Changing op field to 2")
        arp_header.op = 2
        print()
        # print(pkt[14:42])
        # print(bytes(arp_header))
        print()
        arp_header = ARP(bytes(arp_header))
        print("ARP src IP: ", arp_header.ip_src)
        print("ARP src MAC: ", arp_header.mac_src)
        print("ARP target IP: ", arp_header.ip_target)
        print("ARP target MAC: ", arp_header.mac_target)
        print("ARP op code: ", arp_header.op)
        print()
        modified_pkt += bytes(arp_header)
        print()
        # print(modified_pkt)
        modified_pkt = scapy.all.Ether(modified_pkt)
        pkt = scapy.all.Ether(pkt[0:42])
        print("Unmodified packet")
        pkt.show()
        print("Modified pkt")
        modified_pkt.show()
        scapy.all.sendp(pkt)
        scapy.all.sendp(modified_pkt)
        break
